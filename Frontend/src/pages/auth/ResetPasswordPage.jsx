...
const ResetPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const passwordValidationMessage = checkValidData("test@example.com", newPassword);
    if (passwordValidationMessage && passwordValidationMessage !== "❌ Email is not valid!") {
    ...
    try {
      ...
      if (res.data.success) {
        setSuccessMessage(res.data.message || "Password has been successfully reset. You can now log in.");
        setNewPassword('');
        setConfirmNewPassword('');
        setTimeout(() => navigate("/"), 3000);
      } else {
        setErrorMessage(res.data.message || "Failed to reset password. Token might be invalid or expired.");
      }
    } catch (error) {
  ...
  if (errorMessage && !token) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-red-100 to-orange-200 p-4 sm:p-8 text-center">
        <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full">
          <h2 className="text-3xl font-bold text-red-700 mb-6">Error</h2>
      ...
      >
        ...
        <div className="mb-4 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-3 focus:ring-indigo-400 focus:border-indigo-500 transition duration-200 ease-in-out text-gray-800 placeholder-gray-500 text-lg"
            required
          />
          <span
            className="absolute right-3 top-1/2 transform -translate-y-1/2 cursor-pointer text-gray-500 hover:text-indigo-600 transition duration-150"
        ...
        <div className="mb-6 relative">
          ...
          <input
            ...
            onChange={(e) => { setConfirmNewPassword(e.target.value); setErrorMessage(null); }}
            className="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-3 focus:ring-indigo-400 focus:border-indigo-500 transition duration-200 ease-in-out text-gray-800 placeholder-gray-500 text-lg"
            required
          />
          <span
        ...
        >
          {isLoading ? (
            <>
              <ImSpinner2 className="animate-spin" size={20} /> Resetting...
            </>
          ) : (
            "Reset Password"
          )}
