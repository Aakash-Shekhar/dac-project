...
const VerifyEmailPage = () => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/auth/verify-email-otp",
        { email, otp },
      ...
      if (res.data.success) {
        ...
        setErrorMessage(res.data.message || "OTP verification failed. Please try again.");
      }
    } catch (error) {
      const errMsg = error.response?.data?.message || "OTP verification failed. Please try again later.";
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/auth/send-verification-otp",
        { email },
        { withCredentials: true }
      );
      ...
      if (res.data.success) {
        ...
        setErrorMessage(res.data.message || "Failed to resend OTP. Please try again.");
      }
    } catch (error) {
      const errMsg = error.response?.data?.message || "Failed to resend OTP. Please try again later.";
      setErrorMessage(errMsg);
  ...
  if (!email && !isLoading && !isResending && !successMessage && !errorMessage) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4 sm:p-8 text-center">
        <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full">
          <h2 className="text-3xl font-bold text-red-700 mb-6">Error</h2>
          <p className="text-gray-700 mb-6">Email information is missing. Please return to registration.</p>
          <button
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-green-100 to-teal-200 relative overflow-hidden p-4 sm:p-8">
      ...
      >
        ...
        <p className="text-center text-gray-700 mb-6">
          An OTP has been sent to <span className="font-semibold text-blue-700">{email}</span>. Please enter it below.
        </p>
