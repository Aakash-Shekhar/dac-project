...
const BudgetList = ({ budgets, categories, onBudgetDeleted, onBudgetUpdated }) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (!editFormData.limit || !editFormData.category || !editFormData.period || !editFormData.startdate || !editFormData.enddate) {
    ...
    try {
      const res = await axios.put(`http://localhost:5000/budgets/${editingBudgetId}`, {
        ...editFormData,
  ...
  return (
    <div>
      ...
      {budgets.length === 0 ? (
        <p className="text-gray-500 italic text-center py-4">No budgets set yet. Set one to see it here!</p>
      ) : (
            ...
            >
              {editingBudgetId === b._id ? (
                <form onSubmit={handleEditSubmit} className="w-full space-y-2">
                  <div>
                    <label htmlFor="edit-budget-category" className="sr-only">Category</label>
                    <select
                  ...
                  <div>
                    <label htmlFor="edit-budget-limit" className="sr-only">Limit</label>
                    <input
                  ...
                  <div>
                    <label htmlFor="edit-budget-period" className="sr-only">Period</label>
                    <select
                  ...
                  <div>
                    <label htmlFor="edit-budget-startdate" className="sr-only">Start Date</label>
                    <input
                  ...
                  <div>
                    <label htmlFor="edit-budget-enddate" className="sr-only">End Date</label>
                    <input
                  ...
                  <div className="flex justify-end space-x-2 mt-3">
                    ...
                    <button
                      ...
                      disabled={editLoadingId === b._id}
                    >
                      {editLoadingId === b._id ? <ImSpinner2 className="animate-spin" /> : <FiEdit size={16} />} Update
                    </button>
                  </div>
                </form>
              ) : (
                <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center">
                  ...
                  <div className="flex space-x-2">
                    ...
                    >
                      {deleteLoadingId === b._id ? <ImSpinner2 className="animate-spin" /> : <FiTrash2 size={18} />}
                    </button>
                  </div>
                </div>
              )}
